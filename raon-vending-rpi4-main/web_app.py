...
@app.route('/api/machines/<machine_id>/stock-summary')
def api_machine_stock_summary(machine_id):
    """API: Return low-stock and out-of-stock counts and item lists for a machine."""
    try:
        machine = Machine.query.filter_by(machine_id=machine_id).first()
        if not machine:
            return jsonify({'error': 'Machine not found'}), 404
        ...
        items = Item.query.filter_by(machine_id=machine.id).all()
        low_stock_items = [i.name for i in items if i.quantity <= i.low_stock_threshold]
        out_of_stock_items = [i.name for i in items if i.quantity == 0]
        ...
        logger.error(f"Error getting machine stock summary: {e}")
        return jsonify({'error': str(e)}), 500
