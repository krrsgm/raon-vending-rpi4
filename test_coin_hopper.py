...
class CoinHopperRelayTest:
    ...
    def relay_on(self):
        """Turn on both motors via CLOSE commands (1-peso and 5-peso hoppers).
        
        Returns:
            True if successful, False otherwise
        """
        try:
            # Close both hoppers (turn on motors)
            ...
            if (response1 and "OK" in response1) and (response2 and "OK" in response2):
                ...
                print("[CoinHopperTest] Relays turned ON (CLOSE 1 and 5)")
                return True
            else:
    ...
    def relay_off(self):
        """Turn off both motors via OPEN commands (1-peso and 5-peso hoppers).
        
        Returns:
            True if successful, False otherwise
        """
        try:
            # Open both hoppers (turn off motors)
            response1 = self.hopper.send_command("OPEN 1")
            response2 = self.hopper.send_command("OPEN 5")
            
            if (response1 and "OK" in response1) and (response2 and "OK" in response2):
                self.relay_active = False
                print("[CoinHopperTest] Relays turned OFF (OPEN 1 and 5) - 5 coins detected!")
                return True
            else:
