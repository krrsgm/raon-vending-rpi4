/*
  ...
  ESP32 sketch to control two coin hoppers (1-peso and 5-peso) over USB serial (Serial).
  The sketch listens on Serial for simple text commands from the Raspberry Pi
  (connected via USB) and controls motor outputs while counting coins via sensor inputs.
  ...
  Adjust `ONE_MOTOR_PIN`, `FIVE_MOTOR_PIN`, `ONE_SENSOR_PIN`, `FIVE_SENSOR_PIN` to match wiring.
*/
...
const int ONE_MOTOR_PIN = 12;   // 1-peso motor control (to driver input)
const int FIVE_MOTOR_PIN = 13;  // 5-peso motor control
...
volatile unsigned int one_count = 0;
volatile unsigned int five_count = 0;
...
void processLine(String line, Stream &out) {
  ...
  if (cmd == "DISPENSE_AMOUNT"){
    if (parts.size() >= 2){
      ...
      if (parts.size() >= 3) tmo = (unsigned long) parts[2].toInt();
    ...
    if (parts.size() >= 3){
      ...
      unsigned long tmo = 30000;
      if (parts.size() >= 4) tmo = (unsigned long) parts[3].toInt();
    ...
    if (parts.size() >= 2){ int denom = parts[1].toInt(); if (denom == 1){ start_motor(ONE_MOTOR_PIN); out.println("OK OPEN ONE"); } else if (denom == 5){ start_motor(FIVE_MOTOR_PIN); out.println("OK OPEN FIVE"); } else out.println("ERR bad denom"); }
  } else if (cmd == "CLOSE"){
    if (parts.size() >= 2){ int denom = parts[1].toInt(); if (denom == 1){ stop_motor(ONE_MOTOR_PIN); out.println("OK CLOSE ONE"); } else if (denom == 5){ stop_motor(FIVE_MOTOR_PIN); out.println("OK CLOSE FIVE"); } else out.println("ERR bad denom"); }
  } else if (cmd == "STATUS"){
    report_status(out);
  } else if (cmd == "STOP"){
...
void setup(){
  ...
  attachInterrupt(digitalPinToInterrupt(ONE_SENSOR_PIN), one_sensor_isr, RISING);
  attachInterrupt(digitalPinToInterrupt(FIVE_SENSOR_PIN), five_sensor_isr, RISING);
  ...
  delay(50);
  Serial.println("ESP32 Coin Hopper ready (USB)");
}
