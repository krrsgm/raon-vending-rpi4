...
class AssignItemsScreen(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # cache structure: idx -> {'path': path, 'img': PhotoImage}
        self._thumb_cache = {}
    ...
    def refresh_slot(self, idx):
        ...
        # handle thumbnail if available, and bust cache when image path changes
        img_path = ''
        try:
        ...
        cache = self._thumb_cache.get(idx)
        cached_path = cache['path'] if cache else None
        need_reload = (img_path != cached_path)

        if not data:
            # clear cache when slot emptied
        ...
        if img_path and PIL_AVAILABLE and os.path.exists(img_path):
            try:
                if need_reload:
                    ...
                    self._thumb_cache[idx] = {'path': img_path, 'img': img}
                img = self._thumb_cache[idx]['img']
