...
class KioskFrame(tk.Frame):
    ...
    def create_item_card(self, parent, item_data):
        ...
        # Right side: placeholder to preserve spacing. The inline quantity selector
        # and 'Add' button have been removed from item cards. Users must click
        # the item to open its detail screen to select quantity and add to cart.
    ...
    def create_widgets(self):
        ...
        # Stock summary badges (Low stock / Out of stock)
        self.low_stock_badge = tk.Label(right_frame, text='', bg='#f39c12', fg='white', font=self.fonts['control_small'], padx=8, pady=6)
        self.low_stock_badge.pack(side='right', padx=(6,0))
        self.out_stock_badge = tk.Label(right_frame, text='', bg='#e74c3c', fg='white', font=self.fonts['control_small'], padx=8, pady=6)
        self.out_stock_badge.pack(side='right', padx=(6,0))
    ...
    def populate_items(self):
        ...
        # Update stock summary badges after items are populated
        try:
    ...
    def update_stock_summary(self):
        """Updates the header badges showing counts for low-stock and out-of-stock items."""
        try:
            items = list(getattr(self.controller, 'items', []) or [])
            low_threshold = 3
            low_count = 0
            out_count = 0
            for it in items:
            ...
            # Update badge text
            self.low_stock_badge.config(text=f'Low: {low_count}')
            self.out_stock_badge.config(text=f'Out: {out_count}')
        except Exception as e:
            print(f"[KioskFrame] Failed to update stock summary: {e}")
