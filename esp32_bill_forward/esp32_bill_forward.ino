...
unsigned int map_count_to_amount(unsigned int cnt) {
  ...
  // Fallback: many TB74 units emit a number of pulses proportional to the
  // denomination (for example 5 pulses for ₱50, 10 pulses for ₱100). If no
  // explicit mapping is configured above, treat each pulse as a 10-peso unit
  // as a sensible default so that 5 pulses -> 50, 10 pulses -> 100, etc.
  return cnt * 10;
}
